 ...
 * Fetches ALL incomplete tasks + completed tasks from last N days
 * This prevents storage bloat from years of completed tasks
 * @param {string} listId - Task list ID
 * @param {number} completedDaysLimit - Days to look back for completed tasks (default 90)
 * @returns {Promise<Array>} Array of task objects
 */
export async function fetchTasksWithCompletedLimit(listId, completedDaysLimit = COMPLETED_TASKS_DAYS_LIMIT) {
  const token = await getAuthToken(false);
  if (!token) throw new Error('NO_AUTH_TOKEN');

  const allTasks = [];
  const completedMinDate = new Date();
  completedMinDate.setDate(completedMinDate.getDate() - completedDaysLimit);
  const completedMin = completedMinDate.toISOString();

  console.log(`[Tasks API] Fetching tasks for list ${listId}:`);
  console.log(`  - All incomplete tasks (no time limit)`);
  console.log(`  - Completed tasks since ${completedMin} (${completedDaysLimit} days)`);

  // PASS 1: Fetch all incomplete tasks (no time limit)
  let pageToken = null;
  let incompleteFetched = 0;
  do {
    const url = new URL(`${API_BASE_URL}/lists/${listId}/tasks`);
    url.searchParams.set('maxResults', '100');
    url.searchParams.set('showCompleted', 'false'); // Only incomplete tasks
    ...
    incompleteFetched += incompleteTasks.length;

    pageToken = data.nextPageToken;

    // Safety limit
    if (allTasks.length >= MAX_TASKS_PER_LIST) {
      console.warn(`List ${listId} has ${MAX_TASKS_PER_LIST}+ incomplete tasks, limiting to prevent storage bloat`);
      break;
    }
  } while (pageToken);

  // PASS 2: Fetch recent completed tasks (last N days)
  pageToken = null;
  ...
  do {
    ...
    url.searchParams.set('showHidden', 'false');
    url.searchParams.set('completedMin', completedMin); // Only completed after this date
    ...
    completedFetched += completedTasks.length;

    pageToken = data.nextPageToken;

    // Safety limit
    if (allTasks.length >= MAX_TASKS_PER_LIST) {
      console.warn(`List ${listId} has ${MAX_TASKS_PER_LIST}+ total tasks, limiting to prevent storage bloat`);
      break;
    }
  } while (pageToken);

  console.log(`[Tasks API] Fetched ${incompleteFetched} incomplete + ${completedFetched} completed = ${allTasks.length} total tasks`);
  ...
  for (const list of lists) {
    try {
      ...
      tasks.forEach((task) => {
        ...
        // CRITICAL FIX: Store BOTH encoded and decoded task ID formats
        // Google Tasks API may return base64-encoded IDs, but Google Calendar DOM
        // format is inconsistent. By storing both formats, we ensure compatibility
        // regardless of which format is used in the DOM.
        ...
        try {
          ...
          // This happens when task.id is not base64 encoded
        }
      });
  ...
  for (const list of lists) {
    try {
      ...
      updatedTasks.forEach((task) => {
        // CRITICAL FIX: Store BOTH encoded and decoded task ID formats
        // Same logic as buildTaskToListMapping() to ensure consistency

        if (task.deleted) {
          // Remove deleted tasks from mapping (both formats)
          if (updatedMapping[task.id]) {
            delete updatedMapping[task.id];
            updatedCount++;
          }

          // Also try to remove the decoded version
          try {
            const decodedId = atob(task.id);
            if (decodedId !== task.id && updatedMapping[decodedId]) {
              delete updatedMapping[decodedId];
              updatedCount++;
            }
          } catch (e) {
            // Decode failed - that's okay
          }
        } else {
          // Add or update task in mapping (including completed tasks for styling)
          // Store original ID
          updatedMapping[task.id] = list.id;
          updatedCount++;
